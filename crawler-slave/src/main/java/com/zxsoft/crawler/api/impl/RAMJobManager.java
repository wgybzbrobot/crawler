...
public class RAMJobManager implements JobManager {
	 ...
	 */
	@Override
	public JobCode create(Map<String, Object> args)
	        throws Exception {
		JobWorker worker = new JobWorker(args);
	...
	private class JobWorker implements Runnable {
		...
		JobWorker(Map<String, Object> args)
		        throws Exception {
		        String type = (String) args.get(Params.JOB_TYPE);
	                JobType _jobType = JobType.valueOf(type.toUpperCase());
		...
		public void run() {
		        ...
		        try {
		                ...
		                LOG.warn("Create Network Search Task failed.", e);
		        }
		        /*
		         * 1. 将任务列表`JHRW_RWLB`中对应任务记录删除
		         * 2. 将任务执行表`JHRW_RWZX`中对应任务记录的机器号字段置为本机器
