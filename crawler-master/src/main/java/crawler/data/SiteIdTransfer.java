...
public class SiteIdTransfer {
	...
	private static Logger LOG = LoggerFactory.getLogger(SiteIdTransfer.class);
	...
	public static void importTid(JdbcTemplate mysqlJdbcTemplate, JdbcTemplate oracleJdbcTemplate, String category,
			int num) {

		List<Website> list = mysqlJdbcTemplate.query("select a.id, a.site, a.comment, a.tid "
				+ " from website a, section b where a.id = b.site and b.category = ? ", new Object[] { category },
				new RowMapper<Website>() {
					@Override
					public Website mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new Website(rs.getInt("id"), rs.getString("site"), rs.getString("comment"), rs
								.getInt("tid"));
					}
				});
		...
		for (Website website : list) {
			...
			List<Map<String, Object>> _list = oracleJdbcTemplate.query("select id, zdmc from fllb_cjlb "
					+ " where zdmc=? and zdbs=? and shzt=1 and bz=0 ", new Object[] { website.getComment(), num },
			...
			mysqlJdbcTemplate.update("update website set tid= ? where id = ? ",
					new Object[] { _map.get("id"), website.getId() });
	...
	public static int mysqlHas(JdbcTemplate mysqlJdbcTemplate, JdbcTemplate oracleJdbcTemplate, String category, int num) {
		List<Website> list = mysqlJdbcTemplate.query("select a.id, a.site, a.comment, a.tid "
				...
				new Object[] { category }, new RowMapper<Website>() {
					@Override
					public Website mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new Website(rs.getInt("id"), rs.getString("site"), rs.getString("comment"), rs
								.getInt("tid"));
		...
		for (Website website : list) {
			...
			List<Map<String, Object>> _list = oracleJdbcTemplate.query("select id, zdmc from fllb_cjlb "
					+ " where zdmc=? and zdbs=? and shzt=0  ", new Object[] { website.getComment(), num },
					new RowMapper<Map<String, Object>>() {
						@Override
						public Map<String, Object> mapRow(ResultSet rs, int arg1) throws SQLException {
							Map<String, Object> map = new HashMap<String, Object>();
	 ...
	 * @param oracleJdbcTemplate
	 */
	public static int oracleHas(JdbcTemplate mysqlJdbcTemplate, JdbcTemplate oracleJdbcTemplate, String category,
			int num) {
		...
		for (Website website : list) {
			List<Website> _list = mysqlJdbcTemplate.query("select a.id, a.site, a.comment, a.tid "
					...
					new Object[] { category, website.getComment() }, new RowMapper<Website>() {
						@Override
						public Website mapRow(ResultSet rs, int rowNum) throws SQLException {
							return new Website(rs.getInt("id"), rs.getString("site"), rs.getString("comment"), rs
									.getInt("tid"));
