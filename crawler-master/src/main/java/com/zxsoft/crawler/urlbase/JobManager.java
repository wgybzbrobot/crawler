...
public class JobManager {
		...
		try {
			while (begin < count) {
				...
				for (String json : set) {
					...
					if ((url.equals(_url) && jobConf.isRecurrence() == true && jobConf.isRecurrence() == jc
							.isRecurrence()) || jobConf.getJobId() == jc.getJobId()) {
	 ...
	 */
	public void submitToWorker(JobConf jobConf) throws CrawlerException, InterruptedException, ExecutionException,
		...
		while (flag) {
			...
			String url = "http://" + worker.getHostPort() + "/" + SlavePath.PATH + "/" + SlavePath.JOB_RESOURCE_PATH;
	...
	public void delete(int jobId) throws CrawlerException {
		...
		if (!found)
			throw new CrawlerException(ErrorCode.SYSTEM_ERROR, "Job not found with jobId:" + jobId);
	}
}
