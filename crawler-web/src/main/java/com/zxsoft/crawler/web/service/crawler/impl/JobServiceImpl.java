...
public class JobServiceImpl implements JobService {
					...
					if (json.contains(query)) {
						try {
							...
							if (p >= start) {
								...
								if (job.getUrl().equalsIgnoreCase(query) || job.getSource_name().contains(query)
	...
	public JobConf querySourceId(int tid) throws CrawlerException {
		List<JobConf> list = oracleJdbcTemplate.query("select sourceid ,zdbs platform from  fllb_cjlb  where id = ? ",
