...
public class ListConfigVerification extends ParseTool {
    ...
    public Map<String, Object> verify(ConfList listConf, String keyword,
        ...
        if (protocolOutput == null || protocolOutput.getStatus() == null
                        || !protocolOutput.getStatus().isSuccess()) {
                ...
                if (StringUtils.isEmpty(listConf.getListdom())) {
                    ...
                    if (CollectionUtils.isEmpty(elements)) {
                        ...
                        if (StringUtils.isEmpty(listConf.getLinedom())) {
                            ...
                            Elements lineElements = listElement.select(listConf
                                            .getLinedom());
                            if (CollectionUtils.isEmpty(lineElements)
                                            || lineElements.size() < 3) {
                                ...
                                for (Element lineEle : lineElements) {
                                    i++;
                                    if (CollectionUtils.isEmpty(lineEle.select(listConf
                                                    .getUrldom()))) {
                                        if (i < 10) {
                                            continue;
                                        }
                                    ...
                                    Element urlEle = lineEle.select(listConf.getUrldom())
                                                    .first();
                                    if (urlEle == null) {
                                        continue;
                                    }
                                    ...
                                    if (!StringUtils.isEmpty(listConf.getUpdatedom())) {
                                        Elements dateElements = lineEle.select(listConf
                                                        .getUpdatedom());
                                        if (CollectionUtils.isEmpty(dateElements)) {
                                            updateErrorCount++;
                                        } else {
                                            update = DateExtractor.extract(dateElements
                                                            .first().html());
                                            if (update == null) {
                                                LOG.warn("没有获取到时间: "
                                                                + dateElements.first()
                                    ...
                                    if (!StringUtils.isEmpty(listConf.getDatedom())) {
                                        ...
                                        if (CollectionUtils.isEmpty(dateElements)) {
                                            ...
                                            releaseDate = DateExtractor
                                                            .extract(dateElements.first()
                                                                            .html());
                                            if (releaseDate == null) {
                                                LOG.warn("没有获取到时间: "
                                                                + dateElements.first()
                                    ...
                                    if (!StringUtils.isEmpty(listConf.getAuthordom())) {
                                        Elements authorEles = lineEle.select(listConf
                                                        .getAuthordom());
                                    ...
                                    if (!StringUtils.isEmpty(listConf.getSynopsisdom())) {
                                        Elements synoEles = lineEle.select(listConf
                                                        .getSynopsisdom());
