...
public class HttpFetcher {
    ...
    public ProtocolOutput fetch(String url, NameValuePair[] data)
                    throws IOException {
    ...
    public ProtocolOutput fetch(WebPage page) {
        ...
        try {
            ...
            String msg = "Fetch " + url + " failed with protocol exception: "
                            + e.getMessage();
            protocolOutput.setStatus(new ProtocolStatus(url,
                            STATUS_CODE.FAILED, msg));
        } catch (IOException e) {
            String msg = "Fetch " + url + " failed with io exception: "
                            + e.getMessage();
            protocolOutput.setStatus(new ProtocolStatus(url,
                            STATUS_CODE.FAILED, msg));
            LOG.error(msg);
        } catch (Exception e) {
            String msg = "Fetch " + url
    ...
    public ProtocolOutput fetchLastPage(WebPage page)
                    throws PageBarNotFoundException {
        if (!page.isAjax()) {
