...
public class NewsParser extends Parser {
    ...
    public NewsParser(RecordInfo recordInfo, DetailRule detailRule, Long prevFetchTime,
                    ExtInfo extInfo) {
    ...
    public FetchStatus parse() throws CrawlerException {
        ...
        if (!StringUtils.isEmpty(replyNumDom)
            ...
            info.setComment_count(ExtExtractor.extractReplyNum(document
                            .select(replyNumDom).first().text()));
